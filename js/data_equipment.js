...
NDX._sideNode = function (layer, type) {
  ...
  if (type === 'trial') {
    ...
    // 【2026-09-13 修复】原 name 恒为占位词「劫难」，而 _assignTrialDiffs 只对 act>=2 回填难名、
    //   第 1 章完全跳过 → 第一章地图上所有劫难都叫「劫难」。此处按全局难号直接从 TRIAL_LIB 取名。
    const _tpl = NDX.TRIAL_LIB && NDX.TRIAL_LIB[d];
    return {
      type, diff: d, drop: pick.drop, material: pick.material,
      name: (_tpl && _tpl.name) || '劫难',
      title: (_tpl && _tpl.title) || ('第' + d + '难'),
    };
  }
  if (type === 'event') {
