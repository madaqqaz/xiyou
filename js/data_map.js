...
NDX._convertStoryTrialsToEvents = function (layers, act) {
  ...
  Object.keys(map).forEach((trialNo) => {
    ...
    // 【2026-09-13 修复】该层若为融合弧承载层（复合节点），不得改写：
    //   弧的子难由 _compoundNext 逐难推进，此处强行转「缘」会把整个复合节点整块吃掉
    //   （实测 ch9 第 1 行弧 [73,74] 被难73 的缘转换覆盖，难73/74 双双从地图消失）。
    //   难73「化电归真」本身无战斗选项，在弧内以劫难形式呈现与「缘」体验一致。
    if (cols.some((c) => layer[c] && layer[c].type === 'compound')) return;
...
// 【2026-09-14 P1 整改·事件曝光率】原为「每次建图重置」——_buildRegionSegment 是按章调用的，
//   等于每章一个独立已用池，同一条奇遇会在一局里的不同章节重复出现（单局只露 9 条里还有重复）。
//   现改为「每局重置一次」：act===1 建图时清空，之后跨章累积，保证一局内 13 个缘节点抽到的
//   是 13 条互不相同的奇遇（events.js 的 pickEvent 本就是「未见过优先」，池空才回落全池）。
if (act === 1) NDX._usedEventIds = [];   // 新局开始（第 1 章建图）才重置缘池已用 id
...
const _plan = _isCh1 ? NDX.MAP_PLAN_CH1 : NDX.MAP_PLAN;
  ...
  for (let L = 1; L <= NDX.LAYER_COUNT; L++) {
    ...
    // 【2026-09-13 劫难融合】原 `L <= 4` 把 L1-L4 锁成四层单列序章（难1/2/3 各占一层，玩家三步零选择）。
    //   现只保留 L2 = 长安送行（观音赠宝五步教学，不可并入融合弧）；
    //   L1 交给下方 fusionAtLayer 生成复合节点「江流儿劫」（承载难1/2/3，逐难推进，每难仍弹六道抉择）。
    if (_isCh1 && L === 2) {
      const _fixedNodes = {
        1: { type: 'trial', name: '金蝉遭贬', title: '第1难·金蝉遭贬', icon: '🪞', diff: 1, fixedTrial: 1, startPoint: true, skipPostProcess: true, next: [2] },
        2: { type: 'event', name: '长安送行', title: '长安送行·观音赠宝', icon: '🏮', diff: 4, songEvent: true, skipPostProcess: true },
        3: { type: 'trial', name: '满月抛江', title: '第3难·满月抛江', icon: '🌊', diff: 3, fixedTrial: 3, skipPostProcess: true, next: [2], skipChoice: true },
        // 4: 原「长安送行」占位——融合后长安送行前移至 L2，此条不再命中（保留仅为存档对照）。
      };
    ...
    if (_fus) {
      ...
      if (L > 1 && L < NDX.LAYER_COUNT) {
        ...
        sideCols.forEach((c) => {
          // 【PHASE 8 S3】弧层旁列同受 L≤2 禁则：开局两层不出精英/土地庙。
          const _t = NDX._pick(L <= 2
            ? ['mob', 'mob', 'mob', 'event', 'event', 'shop']
            : ['mob', 'mob', 'event', 'event', 'rest', 'elite']);
    ...
    cols.forEach((c) => {
      ...
      if (c === mainC) {
        if (L === NDX.LAYER_COUNT) {
          ...
          //   章内行数 > 9 的章（第 1 章 11 行、第 8 章 10 行）会命中 tpl = _plan[8] 这个 Boss 占位，
          //   照搬就会在章中凭空多出一个关隘 Boss（原第 8 章 14 行时第 9 行确实多出 1 个 Boss，共 2 个；
          //   PHASE 6 融合后该章为 10 行，仍 > 9，本分支依旧生效）。
        ...
        if (L <= 5) {
          ...
          // 【PHASE 8 S3 节奏禁则】L≤2 禁精英、禁土地庙（杀戮尖塔「前 5 行禁精英/营火」的层制弱化版）：
...
NDX._enforceMapRules = function (layers, off) {
  ...
  // 4)【PHASE 8 S2】每章宝窟保底 ≥2（杀戮尖塔「中点全宝箱行 F9」的本土化）：
//    V9.8 宝窟回归后仅在后段岔路池低权重投放，实测全 9 章 209 格仅 2 个宝窟（1%），
//    玩家常整章零法宝收入。不足时从末层向上把「非锁定、非功能房」的岔路格改为宝窟
//    （_replaceNode 走 _sideNode → 自动按 TREASURE_LUX_CHANCE 概率升格秘藏宝窟）。
  const _treCount = [];
  for (let L = 1; L <= LC; L++) colsOf(L).forEach((c) => {
    const _t = layers[L][c] && layers[L][c].type;
    if (_t === 'treasure' || _t === 'treasure_lux') _treCount.push(1);
  });
  let _needTre = 2 - _treCount.length;
  //   下限 L>=2：4 层小章（ch7 LC=4）的 LC-1=3 < 4 会跳过整段，宝窟保底永不执行（实测 ch7=0）。
  for (let L = LC - 1; L >= 2 && _needTre > 0; L--) {
    const cands = colsOf(L).filter((c) => {
      const n = layers[L][c];
      if (!n || n.fixedTrial || n.skipPostProcess || n.songEvent || n.fixedEventTrial) return false;
      if (n.type === 'compound' || n.type === 'boss' || n.type === 'rest') return false;
      if (n.type === 'treasure' || n.type === 'treasure_lux') return false;
      return true;
    });
    cands.sort((a, b) => (layers[L][a].type === 'mob' ? 0 : 1) - (layers[L][b].type === 'mob' ? 0 : 1));
    if (cands.length) { NDX._replaceNode(layers, L, cands[0], 'treasure', off); _needTre--; }
  }
};
...
NDX._ensureHiddenTrials = function (layers, act, off) {
  ...
  needed.forEach((g) => {
    ...
    // 【2026-09-13 PHASE 7】层号定位：compact 章的难号由游标分配，不再满足线性 L = g - lo。
    //   先查「哪个层已绑该难号」（_assignTrialDiffs 已写入 fixedTrial）——命中说明已被承载，
    //   交给下方 hasTrial 判断自然 return；未命中再回退线性映射（非 compact 章行为不变）。
    let L = 0;
    for (let i = 1; i < NDX.LAYER_COUNT; i++) {
      const _row = layers[i] || {};
      if (Object.keys(_row).some((c) => _row[c] && +_row[c].fixedTrial === +g)) { L = i; break; }
    }
    if (!L) L = g - lo;               // 局部层（1~LAYER_COUNT）
...
NDX._assignTrialDiffs = function (layers, act, off) {
  // 【2026-09-13 修复】原实现 `act === 1` 直接 return，导致第 1 章的劫难位从不绑定真实难号/难名，
  //   地图上所有劫难节点都只显示占位词「劫难」。固定序章（skipPostProcess / songEvent / startPoint）
  //   已被下方 isLocked 保护，第 1 章参与收束不会改写序章与长安送行。
  if (!act) return;
  const LC = NDX.LAYER_COUNT;
  ...
  // 融合弧真正承载的难号 = 各弧 fusions[].diffs 的并集。
  //   不可用 compoundFor(act).diffs —— 自动生成器把该兼容字段写成「本章全部非 Boss 难」
  //   （见 data_compound.js 的 quotaTier 分支），当排除集用会把本该由普通探索层承载的
  //   难号一并排除掉（实测 ch8 因此漏承载 65-71 共 7 难）。
  const _fc = NDX.compoundFor(act);
  const _fusList = (NDX.regionFusions && NDX.regionFusions(act)) || [];
  const _fusedSet = new Set();
  _fusList.forEach((f) => (f.diffs || []).forEach((d) => _fusedSet.add(d)));  // 融合弧已承载的难号
  const isLocked = (n) => !n || !!(n.skipPostProcess || n.songEvent || n.fixedEventTrial
    || n.startPoint || n.type === 'compound' || n.type === 'boss' || n.type === 'branch');
  // 【2026-09-13 PHASE 7】紧凑分配（opt-in：COMPOUND_NODES[act].compact === true）
  //   线性映射 g = lo + L - 1 把「层号 → 难号」钉死。融合弧一旦吃掉某层的 g，该层只能降级为
  //   纯填充层，且后续层的 g **不会**自动前移——于是「加弧」不但不省层，反而凭空多出空层
  //   （实测 ch2 按线性法加 [14,15] / [16,17] 两弧，L3/L4 会空转成两个无劫难层，9 层还是 9 层）。
  //   紧凑模式换成「难号游标」：跳过入弧 / 已转缘的难号，把剩余难号按顺序填进每个可承载层。
  //   于是 layers 得以真正小于章内难数，且不留空层（ch2：9 层 → 7 层）。
  const _compact = !!(_fc && _fc.compact);
  let cursor = lo;
  const _advance = () => { while (cursor < hi && (_fusedSet.has(cursor) || fixedEventSet.has(cursor))) cursor++; };
  // 该行非锁定的劫难位一律降级为小怪（用于弧层 / 线性模式下被弧吃掉的层）
  const _demoteRow = (L, layer) => {
    Object.keys(layer).map(Number).forEach((c) => {
      if (layer[c] && layer[c].type === 'trial' && !isLocked(layer[c])) NDX._replaceNode(layers, L, c, 'mob', off);
    });
  };
  if (_compact) _advance();
  for (let L = 1; L < LC; L++) {
    const layer = layers[L];
    if (!layer) continue;
    ...
    if (_compact) {
      _advance();
      if (cursor >= hi) continue;                // 本地区难号（含末层 Boss）已分配完，本层不设劫难位
      g = cursor;
    } else {
      g = lo + L - 1;                            // 该层应承载的全局难号（既有章节奏不变）
      if (g > hi) break;                         // 超出本地区难号范围
      if (fixedEventSet.has(g)) continue;        // 已转为「缘(?)」事件节点，剧情由事件承载
    }
    // 【2026-09-13 修复】融合弧（复合节点）已承载的难号，本层不得再占劫难位。
    //   原实现只排除 fixedEventSet，导致第 1 章难 10/11/12 由融合弧承载的同时，L11/L12 又各自
    //   绑了一次难 11/难 12——同一难号在同一章被历两次（实测 TRIAL#11 与 COMPOUND[10,11,12] 并存）。
    // 【2026-09-13 修复】融合弧已承载的难号，本层不得再占劫难位（否则同一难号被历两次）。
    //   compact 章的弧难号由 _advance() 在游标层跳过，故此处只管线性模式。
    if (!_compact && _fusedSet.has(g)) { _demoteRow(L, layer); continue; }
    ...
    if (_compact) cursor = g + 1;                // 消费游标（该难号已落到本层劫难位）
  }
};
...
NDX._genBranchCols = function (prevCols) {
  const N = NDX._rand(3, Math.min(5, NDX.MAX_COL)); // 每层 3~5 个节点（PHASE 8：上限 4→5，横向选择深度对齐杀戮尖塔）
  let pool, prefer;
  if (!prevCols.length) {
