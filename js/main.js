...
function startGame() {
  try {
    ...
    // V9.9 开场动画：每次进入自动播放，播放完成后进入游戏
    if (!NDX._introPlayed && window.IntroVideo && typeof window.IntroVideo.play === 'function') {
      NDX._introPlayed = true;
      console.log('[NDX] 播放开场动画...');
      window.IntroVideo.play(function() {
        console.log('[NDX] 开场动画播放完成，进入游戏');
        startGame();
      });
      return;
    }
    
    // V9.8 游戏开始加载动画（替代章节过场动画）
    if (NDX.loadingScreen && typeof NDX.loadingScreen.show === 'function') {
      NDX.loadingScreen.show(function() {
        // 加载动画完成后的回调
        console.log('[NDX] 加载动画完成，游戏开始');
      });
    }
    _hideBuyoutGate();
...
function doRender() {
  ...
  if (!_firstRenderDone) {
    ...
    if (NDX.loadingScreen && typeof NDX.loadingScreen.complete === 'function') {
      setTimeout(function() { NDX.loadingScreen.complete(); }, 500);
    }
  }
}
...
case 'sutra-finish': {
  ...
  break;
    ...
    case 'gy-popup-open': {
      // 观音弹出覆盖层：独立 append 到 body，不嵌入 scene-modal，避免 doRender() 重渲染丢失 show 状态
      const _p = NDX.game && NDX.game.state && NDX.game.state.pending;
      const _txt = (_p && _p.teachText) || '六道抉择，随心而定。';
      if (NDX.ui && NDX.ui.showGuanyinPopup) NDX.ui.showGuanyinPopup(_txt);
      break;
    }
    case 'gy-popup-dismiss': {
      // 观音弹出覆盖层：播出场动画后移除
      if (NDX.ui && NDX.ui.hideGuanyinPopup) NDX.ui.hideGuanyinPopup();
      break;
    }
    case 'scene-next': {
    ...
    case 'replay-intro': {
      try {
        ...
        NDX._introPlayed = false;
      } catch (e) {}
      if (typeof showTip === 'function') {
        showTip('已清除开场动画观看记录，下次进入游戏时自动播放', 'ok');
      } else {
        alert('已清除开场动画观看记录，下次进入游戏时自动播放');
      }
      break;
    }
    // V8.37 无障碍设置
    ...
    case 'play-intro':
      ...
      if (window.IntroVideo) {
        window.IntroVideo.play(function() {
          console.log('[NDX] 宣传动画播放完成');
        });
      } else {
        console.warn('[NDX] IntroVideo module not loaded');
      }
      break;
    case 'open-codex':
    ...
    case 'set-stance': {
      // STANCE·战前攻守姿态：切换并 re-resolve 整场预结算（仅在开场前可改）
      const st = el && el.dataset ? el.dataset.stance : null;
      if (st && g && g.setStance) g.setStance(st);
      g.render();
      break;
    }
    ...
    case 'fate-graph-next':
      // 网状叙事·命运图谱（P3）：点「继续」沿 then 链进入 gameover 胜局屏（论道浮层在此屏拦截）
      if (s && s.pending && s.pending.kind === 'fate-graph') s.pending = s.pending.then || { kind: 'gameover' };
      g.render();
      break;
    case 'narrative-next':
