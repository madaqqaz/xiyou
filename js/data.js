...
NDX._tailStep = function (s) {
  ...
  // 【PHASE 8 S5】延伸层升级：trial 权重 1/4→2/6——实测随机走法仅历 40-62% 难号，其余全靠动态尾硬拖，
//   旧池 mob 过半（50%）使延伸层沦为杂鱼汤，补任务动辄多走 8-12 层；elite/rest 入池，
//   补任务路上也能拿遗物/回血——延伸层从「惩罚」变「有规划价值的支线」。
  const pool = ['mob', 'event', 'trial', 'trial', 'elite', 'rest'];
  const nodeMap = {};
  const cols = [];
  for (let c = 1; c <= cnt; c++) {
...
NDX.buildReturnScene = function (s) {
  if (!s) return null;
  const base = (() => {
    const left = s.life || 0;                  // 到西天时的余寿（年）
    const S = NDX.LIFE.SHRINE_R, P = NDX.LIFE.PERFECT_R;
    // 用 typeof 判定而非 `|| 9` 兜底：否则 RC=0（免返程费）的配置会被误判成 9，
    //   返程费将永远无法被关掉，门禁的反向反证也因此失效。
    const RC = (NDX.LIFE && typeof NDX.LIFE.RETURN_COST === 'number') ? NDX.LIFE.RETURN_COST : 9; // 返程固定耗寿（年）·西天回长安
    // V9.7 天数制 + 【V9.8 返程兑现 2026-09-14】：大限 50、起点 27（余寿 23）。
    //   到西天后【强制扣除】RETURN_COST = 9 年东归——此前该常量定义后从未被消费，
    //   结算直接把「到西天余寿」当「回长安余寿」返回，导致结局文案/排行榜/成就全部虚高 9 年。
    //   R < 9   → 付不起这 9 年返程费，坐化归途、留舍利塔（>41 岁到西天）
    //   R ≥ 14  → 完美（≤36 岁到西天；扣 9 后回长安余 ≥5 年，45 岁归）
    //   9≤R<14  → 不完美（37-41 岁到西天仍回得去，45-50 岁归）
    //   ⚠ years 一律返回【扣减后】的回长安余寿；到西天原值另存 shrineLife 供演出引用。
    if (left < S) return { grade: 'shrine', title: '返程坐化 · 留章舍利', years: Math.max(0, Math.floor(left)), shrineLife: Math.max(0, Math.floor(left)), returnCost: RC, region: NDX.returnShrineRegion(left) };
    const perf = left >= P;
    const after = Math.max(0, left - RC);      // 回长安余寿 = 到西天余寿 − 东归九年
    return { grade: 'return', title: perf ? '回长安 · 受封留名' : '回长安 · 迟归受封', perfect: perf, years: Math.max(0, Math.floor(after)), shrineLife: Math.max(0, Math.floor(left)), returnCost: RC };
  })();
  ...
  return {
    ...
    // 【V9.8 返程兑现】透传返程账目：shrineLife=到西天余寿、years=回长安余寿、returnCost=东归九年。
    //   UI / 排行榜据此可同时展示「到西天剩多少」与「扣九年之后带回长安多少」。
    shrineLife: base.shrineLife, returnCost: base.returnCost,
    honor, closing, redeem,
    acts: [
      { key: '归', text: base.grade === 'shrine'
          ? `转身离了灵山，往东是家。余寿 ${base.shrineLife} 年，凑不够东归那九年——一步一念，念的都是最后一念。`
          : `转身离了灵山，往东是家。到西天时尚余 ${base.shrineLife} 年，东归先扣九年——到长安还剩 ${base.years} 年，一步一念。` },
