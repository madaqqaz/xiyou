    ...
    return {
      ...
      opts: (function () {
        const raw = (t.options || []).map((o) => ({
          ...
          setFlag: o.setFlag || null,           // 网状叙事（P0）：选项结算落印记
          requireFlag: o.requireFlag || null,   // 网状叙事（P0）：前置印记门
          requireLock: o.requireLock || null,   // 网状叙事（P1）：锁定联动（已落印记则反锁）
          requireRel: o.requireRel || null,     // 网状叙事（P1）：NPC关系门槛
          consequence: o.consequence || null,    // 网状叙事（P1）：选项后果提示文案
      ...
      branchKey: t.branchKey || null,   // 网状叙事（P0）：保留分支键供调试（分支已在 enterNode 归一前合并）
