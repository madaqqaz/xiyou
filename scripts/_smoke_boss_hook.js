...
// 【2026-09-13 契约变更】劫难融合后 layers ≠ 章内难数 —— 第 1 章难1-3（金蝉遭贬／出胎几杀／
//   满月抛江，三步纯叙事各只做一次六道抉择）并为一个复合节点「江流儿劫」，layers 由 13 → 11。
//   layers 语义自此为「章内必经节点数」，恒 ≤ 章内难数。原断言要防的「diffOfLayer 断号」
//   （某难既不在层映射里、也永远抽不到 → 该难从地图静默消失）不能靠删断言了事，
//   已升级为 _verify_map_ch1_arc.js 的「难号承载完整性」正面守卫：每章 [start,end] 全部难号
//   必须被 融合弧子难 / 单难位 / 缘事件位 / 关隘 Boss 之一承载，实测建图验证 0 漏承载。
//   此处保留轻量结构守卫：layers 不得失控（上界）；且凡 layers < 难数的紧凑章，
//   其融合弧子难并集必须足以解释「少掉的行数」（防止有人只改 layers 数字而忘了挂弧）。
// 【2026-09-14 契约再变更】layers **可以**大于章内难数了：2026-09-14 补 ch7（4 层 → 9 层，
//   分支度 1.88 → 3.34）后出现了「补层纵深章」——章内难号少（ch7 仅 55/56/57 三个非 Boss 难），
//   靠无劫难位的探索层把体量与分支度撑起来。故上界守卫取代「≤ 章内难数」：
//   层数区间 2~12（下界防单层章，上界防层数失控拖长单局）。
const badLayers = [];
const compactActs = [];
for (let a = 1; a <= NCH; a++) {
  const r = NDX.actRange(a);
  const cnt = r.end - r.start + 1;
  if (r.layers < 2 || r.layers > 12) badLayers.push('act' + a + ' layers=' + r.layers);
  if (r.layers < cnt) compactActs.push(a);
}
ck('各章 layers 落在 2~12（层数失控守卫；layers 可 > 章内难数：补层纵深章如 ch7 9层/4难）',
  badLayers.length === 0, badLayers.join(','));
const badCompact = [];
compactActs.forEach((a) => {
  const fus = NDX.regionFusions(a) || [];
  const u = new Set(); fus.forEach((f) => (f.diffs || []).forEach((d) => u.add(d)));
  const merged = NDX.actTrials(a) - NDX.actLayers(a);   // 被压掉的难数 = 必须由弧解释的规模
  if (u.size < merged) badCompact.push('act' + a + ' 弧并集' + u.size + '<并入难' + merged);
});
ck('紧凑章（layers<难数）确有融合弧承载被并入的难', badCompact.length === 0, badCompact.join(',') || ('紧凑章 ' + compactActs.join(',')));
...
const _f1x = NDX.fusionAtLayer(1, 1);
ck('act1 第 1 行 = 江流儿劫弧（难1-3 融合）', !!_f1x && _f1x.diffs.join(',') === '1,2,3', JSON.stringify(_f1x && _f1x.diffs));
ck('act1 第 2 行留给长安送行，不得挂弧（反证）', !NDX.fusionAtLayer(1, 2));
